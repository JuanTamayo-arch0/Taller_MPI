...
int main(int argc, char** argv) {
    ...
    int size = atoi(argv[1]); // Tamaño del vector obtenido desde la línea de comandos

    // Esperamos: ./prog <size> v0 v1 v2 ... v(size-1)
    if (argc < 2 + size) {
        fprintf(stderr, "Usage: %s <size> <v0> <v1> ... <v(size-1)>\n", argv[0]);
        return 1;
    }

    printf("--------- SortVector&Sequential (CommandLine - explicit data) --------\n");

    // Se declara el vector con tamaño dinámico
    int vector[size];

    // Leer valores del vector desde argv
