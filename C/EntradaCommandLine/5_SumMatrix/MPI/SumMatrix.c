...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        printf("--------- SumMatrix&MPI (CommandLine - explicit data) --------\n");
        if (argc < 3) {
            fprintf(stderr, "Usage: mpirun -np <procs> %s <n> <m> <A_elements> <B_elements>\n", argv[0]);
        ...
        if (argc < 3 + 2 * n * m) {
            fprintf(stderr, "Error: se esperaban %d valores para A y %d para B (2*n*m en total)\n", n*m, n*m);
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
