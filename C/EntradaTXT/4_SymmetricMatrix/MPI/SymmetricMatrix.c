...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        printf("--------- SymmetricMatrix&MPI (Parametros) --------\n");
        ...
        // Reservar memoria y leer matriz original desde params.txt
        matrix_data = malloc(n * m * sizeof(int));
        transpose_data = malloc(n * m * sizeof(int));
        if (!matrix_data || !transpose_data) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (fscanf(f, "%d", &matrix_data[i * m + j]) != 1) {
