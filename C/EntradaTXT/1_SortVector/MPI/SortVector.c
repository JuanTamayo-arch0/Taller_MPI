...
int main(int argc, char** argv) {
    ...
    if (pid == 0) {
        printf("--------- SortVector&Parallel (MPI - Parametros) --------\n");

        // Leer tamaño desde archivo params.txt en la carpeta del ejecutable
        FILE* f = fopen("params.txt", "r");
        if (!f) {
            fprintf(stderr, "params.txt no encontrado. Coloque un archivo con el tamaño en esta carpeta.\n");
        ...
        // Reservar memoria para el vector principal y leer los elementos desde params.txt
    ...
    // Preparar reparto con MPI_Scatterv para manejar tamaños no uniformes
    int *sendcounts = NULL;
    int *displs = NULL;
    if (pid == 0) {
    ...
    int local_size = size / npr + (pid < (size % npr) ? 1 : 0);
